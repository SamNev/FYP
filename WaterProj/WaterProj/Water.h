...
struct Drop {
    ...
    static void cascade(glm::vec2 pos, glm::ivec2 dim, float* h, float* p) {
...
bool Drop::descend(glm::vec3 n, float* h, float* p, float* b, float* track, float* pd, glm::ivec2 dim, float scale) {
    ...
    if (glm::length(glm::vec2(n.x, n.z)) * effF < 1E-5)
        return false;

    speed = glm::mix(glm::vec2(n.x, n.z), speed, effF);
...
bool Drop::flood(float* h, float* p, glm::ivec2 dim) {
    ...
    const std::function<bool(ivec2, float)> findset = [&](ivec2 i, float plane) {
